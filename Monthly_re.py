...
import sqlite3
# Custom modules: data loading, technical indicators, PCA signal generation
...
# ─── Universe Filtering ──────────────────────────────────────────────────────
# Remove illiquid stocks (low volume) and penny stocks (price below $7)
...
rolling_df['daily_return'] = rolling_df['daily_return'].clip(-0.3, 0.3)  # cap extreme daily moves
...
# Sector beta
# Compute each stock's rolling beta relative to its sector's average return
...
# Range compression
# Measures intraday price range as a fraction of close — a proxy for realized vol and liquidity
...
# kc_width
# Keltner Channel width: ATR normalized by price — captures volatility regime
...
# Smooth all factor columns with EWM to reduce noise before PCA
# exponential smoothing
...
# ─── Rebalance Date Setup ────────────────────────────────────────────────────
# Identify rebalance dates (~every 20 trading days) and compute forward returns
rebalance_dates, rolling_df = calculate_future_returns(rolling_df, holding_period_days=20)

# Initialize tracking variables for the backtest loop
...
# ─── Backtest Loop ───────────────────────────────────────────────────────────
# Iterate over each rebalance date, build signals, and compute portfolio returns
transaction_cost = 0.004375  # 0.4375% per trade (round-trip cost assumption)
n=5  # skip first n dates to allow indicator warmup
for rebalance_date in rebalance_dates[n:]:

    # Snapshot of the universe on this rebalance date
    df_slice = rolling_df[rolling_df['date'] == rebalance_date].copy()
    # Filter out stocks with high drawdown risk (top 40% by Ulcer Index removed)
    ...
    # Run PCA on the factor matrix to extract latent components
    factor_loadings = pca_factor_loadings(df_slice, factor_cols, pcs_to_use=slice(None))
    # Compute per-stock residuals after removing PCA-explained variance (idiosyncratic signal)
    residuals_df = get_residuals(factor_loadings, df_slice[['ticker', 'gsector', 'weekly_return_5d']])
    # Neutralize residuals against sector beta to remove market/sector bias
    residuals_df = neutralize_z_residual(residuals_df, df_slice, controls = ['beta_sector'])
    # Blend current residuals with historical alpha for signal smoothing
    residuals_df = composite_alpha(residuals_df, alpha_history)

    # Generate longs using smoothed residual
    # Select top N stocks by composite alpha, with max 1 stock per sector
    ...
    # Halve exposure when the market regime signal is unstable
    ...
    # Gross return from holdings, then subtract transaction costs
    ...
    # On the final rebalance, print the trades to execute next
    if rebalance_date == rebalance_dates[-1]: print('Implement new portfolio on the trading day after:',rebalance_date,'\n', trades)


# ─── Build Daily Returns ─────────────────────────────────────────────────────
# Forward-fill portfolio weights between rebalances to get a daily return series
...
# ─── Align and Normalize ─────────────────────────────────────────────────────
# Merge PCA strategy returns with S&P 500 on common dates
...
# Rebase both series to 1.0 on the same start date for fair comparison
...
# ─── Sharpe Ratio ────────────────────────────────────────────────────────────
# Compute daily returns from cumulative series for Sharpe calculation
