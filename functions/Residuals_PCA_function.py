...
def get_residuals(factor_loadings, returns_df):
    ...
    return merged[['ticker', 'gsector', 'residual']]

def generate_long_only_signals(residuals_df, top_n=25, skip_bottom_n=25, per_sector_limit=3):
    ...
    with warnings.catch_warnings():
        ...
        # Sector neutralization → limit per sector picks
        longs = longs.groupby('gsector', group_keys=False).apply(lambda x: x.head(per_sector_limit)).reset_index(drop=True)
...
def calculate_turnover_and_cost(previous_weights_df, current_weights_df, transaction_cost_per_unit):
    """
    Returns:
